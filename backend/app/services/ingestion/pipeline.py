...
class IngestionPipeline:
    ...
    async def reindex_all_from_db(self, session: AsyncSession) -> int:
        ...
        child_stmt = select(ChildChunk, ParentChunk, Document).join(
            ParentChunk, ChildChunk.parent_chunk_id == ParentChunk.id
        ).join(
            Document, ChildChunk.document_id == Document.id
        )
