...
from backend.app.core.dependencies import IngestionPipeline, get_ingestion_pipeline
    ...
    for file in files:
        try:
            ...
            res = await pipeline.ingest_bytes(
                filename=file.filename or "untitled.txt", raw_bytes=content_bytes, session=session
            )
            results.append(res)
    ...
    for file_path in vault_dir.rglob("*"):
        if file_path.is_file() and file_path.suffix.lower() in [
            ...
            try:
                ...
                res = await pipeline.ingest_bytes(
                    filename=rel_name, raw_bytes=raw_bytes, session=session
                )
                synced_files.append(res)
            except Exception as e:
                synced_files.append(
                    {"filename": file_path.name, "status": "error", "message": str(e)}
                )
...
async def get_document(doc_id: str, session: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    ...
    p_stmt = (
        select(ParentChunk)
        .where(ParentChunk.document_id == doc_id)
